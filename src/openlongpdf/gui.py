...
def render_browser_assist(
    ...
    *,
    ...
    start_url = _chatgpt_extension_start_url(base_url=base_url, token=token)
    fallback_bookmarklet = _build_bookmarklet(base_url=base_url, token=token)
...
<main>
  ...
  <section class="panel">
    <p>Chrome extension installed? Click this button once.</p>
    <p><a class="primary-action" href="{html.escape(start_url)}" target="_blank" rel="noreferrer">Start Translation In ChatGPT</a></p>
  </section>
  <details class="panel">
    <summary>Advanced</summary>
    <p>Install the Chrome extension once: open <code>chrome://extensions</code>, enable Developer mode, choose <strong>Load unpacked</strong>, and select <code>{html.escape(str(extension_dir))}</code>.</p>
    ...
    <p>The extension opens one visible panel on ChatGPT. After one confirmation, it sends each remaining pack, waits for ChatGPT to finish, extracts the new assistant response, posts it back to <code>/assist/import-response</code>, imports translated chunks, and assembles the final notes when all packs are imported.</p>
    <p>It does not store ChatGPT credentials, cookies, access tokens, or session data.</p>
    <p>Fallback without the extension: drag this bookmarklet to the bookmarks bar, open ChatGPT, then click it.</p>
    <p><a class="bookmarklet" href="{html.escape(fallback_bookmarklet)}">OpenLongPDF Bookmarklet</a></p>
    <textarea readonly>{html.escape(fallback_bookmarklet)}</textarea>
...
def _chatgpt_extension_start_url(*, base_url: str, token: str) -> str:
    query = urllib.parse.urlencode(
        {
            "openlongpdf_server": base_url,
            "openlongpdf_token": token,
        }
    )
    return f"{CHATGPT_URL}?{query}"


def _build_bookmarklet(*, base_url: str, token: str) -> str:
    core = _assist_core_script()
    start_url = _chatgpt_extension_start_url(base_url=base_url, token=token)
    script = (
        "(()=>{"
        f"const server={json.dumps(base_url)};"
        f"const token={json.dumps(token)};"
        f"const chatgptUrl={json.dumps(start_url)};"
        "if(location.origin!=='https://chatgpt.com'){"
        "window.open(chatgptUrl,'_blank','noopener,noreferrer');"
        "alert('Open ChatGPT, then click the OpenLongPDF bookmarklet again on that page.');"
        "return;"
        "}"
        f"{core}"
...
def _assist_core_script() -> str:
    path = Path(__file__).resolve().parents[2] / "extension" / "assist-core.js"
